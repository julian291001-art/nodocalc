...
function calcularSistema(nodos: Nodo[], elementos: Elemento[], fuerzas: FuerzaExterna[], modoDiseno: boolean) {
    ...
    nodosConEcuacion.forEach(n => {
      ...
      elementosCalc.forEach(e => {
        ...
        if (angN !== null) {
          ...
          if (e.conocido) { knownX += e.valorConocido * cosA; knownY += e.valorConocido * sinA }
          else {
            const idx = incognitas.findIndex(i => i.tipo === "elemento" && i.refId === e.id)
            rowX[idx] += cosA; rowY[idx] += sinA
          }
        }
      })
    ...
    if (solucion) {
      checkPorNodo = nodosConEcuacion.map(n => {
        ...
        elementosCalc.forEach(e => {
          ...
          if (angN !== null) {
            const idx = incognitas.findIndex(i => i.tipo === "elemento" && i.refId === e.id)
            const val = e.conocido ? e.valorConocido : solucion![idx]
            fx += val * Math.cos(angN * Math.PI / 180)
            fy += val * Math.sin(angN * Math.PI / 180)
          }
        })
        if (n.apoyo === "pasador" || n.apoyo === "empotrado") {
  ...
  return { elementosCalc, incognitas, nodosConEcuacion, numEcuaciones, determinado, solucion, checkPorNodo, cablesInvalidos, disenoResultado }
}
...
export default function Equilibrio2D() {
  ...
  const dibujar = () => {
    ...
    elementosCalc.forEach(e => {
      ...
      ctx.strokeStyle = esInvalido?"#dc2626":e.color; ctx.lineWidth = e.tipo==="barra"?4:2
      ...
      ctx.fillText(`${e.nombre}=${fmt(val)} ${cfg.fuerza}${esInvalido?" ⚠":""}`, tx(A.x)+(tx(B.x)-tx(A.x))*0.45+4, ty(A.y)+(ty(B.y)-ty(A.y))*0.45-4)
    })
    ...
    nodos.forEach(n => {
      ...
      if (n.apoyo==="fijo") {
        ...
        ctx.fillStyle="#94a3b8"; ctx.beginPath();ctx.moveTo(nx,ny);ctx.lineTo(nx-9,ny+16);ctx.lineTo(nx+9,ny+16);ctx.closePath();ctx.fill()
        for (let k=-8;k<=8;k+=4){ctx.strokeStyle="#94a3b8";ctx.beginPath();ctx.moveTo(nx+k,ny+16);ctx.lineTo(nx+k-4,ny+22);ctx.stroke()}
      } else if (n.apoyo==="rodillo") {
        ctx.save();ctx.translate(nx,ny);ctx.rotate(-(n.anguloRodillo*Math.PI/180))
        ...
        ctx.beginPath();ctx.arc(5,18,3,0,Math.PI*2);ctx.fill();ctx.restore()
    ...
    fuerzas.forEach(f => {
      ...
      const len=65
      const fx2=nx+len*Math.cos(angMostrar*Math.PI/180), fy2=ny-len*Math.sin(angMostrar*Math.PI/180)
