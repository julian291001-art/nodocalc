...
export default function Equilibrio2D() {
  ...
  if (determinado) {
    ...
    nodosConEcuacion.forEach(n => {
      ...
      if (n.apoyo === "pasador" || n.apoyo === "empotrado") {
        ...
        rowX[idxR] += Math.cos(angPerp); rowY[idxR] += Math.sin(angPerp)
      }
    ...
    if (solucion) {
      checkPorNodo = nodosConEcuacion.map(n => {
        ...
        if (n.apoyo === "pasador" || n.apoyo === "empotrado") {
          ...
          const angPerp = (n.anguloRodillo + 90) * Math.PI / 180
          fx += solucion![iR] * Math.cos(angPerp)
          fy += solucion![iR] * Math.sin(angPerp)
        }
        fuerzas.filter(f => f.nodoId === n.id).forEach(f => {
          if (f.modo === "completa") {
            ...
            const idx = incognitas.findIndex(i => i.tipo === "fuerzaMag" && i.refId === f.id)
            fx += solucion![idx] * Math.cos(f.angulo * Math.PI / 180)
            fy += solucion![idx] * Math.sin(f.angulo * Math.PI / 180)
          } else {
  ...
  const dibujar = () => {
    ...
    fuerzas.forEach(f => {
      ...
      const len = 65
      const fx2 = nx + len*Math.cos(angMostrar*Math.PI/180)
      const fy2 = ny - len*Math.sin(angMostrar*Math.PI/180)
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <label className="flex items-start gap-2 cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">Los cables se resuelven como incógnitas con W conocida. El sistema calcula el W máximo según la tensión admisible de cada cable.</div>
                  </div>
                </label>
              </div>
                ...
                <div className="flex flex-col gap-2">
                  {elementosCalc.map(e => (
                    <div key={e.id} className="p-3 rounded-xl border border-gray-100 bg-gray-50">
                      ...
                      <div className="text-xs text-gray-400 mb-2">{e.tipo === "cable" ? "Cable" : "Barra rígida"} — longitud: {fmt(e.longitud)} {cfg.longitud} — ángulo: {fmt(e.angDesdeA)}°</div>
